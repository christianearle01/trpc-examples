...
import { FetchUserByEmail, LoginUserParams, RegisterUserParams } from '../config/types';
...
class UsersModel extends DatabaseModel {
    ...
    registerUser = async (params: RegisterUserParams) =>{
        ...
        try{
            ...
            response_data = await this.executeQuery<OkPacketParams>(register_query) as any;
        }
        catch(error){
            console.log(error);
            response_data.error = error;
    ...
    fetchUserByEmail = async (email_address: string, selected_fields: string = '*') => {
        ...
        try{
            ...
            response_data = await this.executeQuery<FetchUserByEmail>(fetch_user_query) as any;
        }
        catch(error){
            console.log(error);
            response_data.error = error;
